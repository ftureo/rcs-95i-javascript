...
// Requisito: Mostrar por consola el nombre de los usuarios en el array Students
// Salida esperada: Fabi[0], Yani[1], Perrito[2]

// Diferentes tipos de variables: const, let y var
// Reasignando el valor por cada iteración
// const: No se puede reasignar el valor
for (let index = 0; index < students.length; index++) {
    console.log(`El index es: ${index}`)
    console.log(students[index].city)
}

// Mostrar por consola el nombre y apellido de los usuarios en el array Students
for (let index = 0; index < students.length; index++) {
    console.log(`El index es: ${index}`)
    console.log(`${students[index].firstname} ${students[index].lastname}`)
}

// Mostrar con un condicional si el usuario tiene una ciudad asignada
for (let index = 0; index < students.length; index++) {
    // if(condition){} else {}
    if (students[index].city) {
        console.log(`El usuario ${students[index].firstname} tiene una ciudad asignada. Esa ciudad es: ${students[index].city}`)
    }
}
...
// sintaxis --> array.forEach(function(currentValue, index, arr), thisValue)
// Cuando a una función le paso como argumento otra función, la función que recibe el argumento se llama callback

// forEach no retonar ningún valor
users.forEach((user) => {
    console.log(user)
})
...
// Map function
// Recorrer todos los elementos (igual que el forEach)
// Pero a diferencia del forEach, el map retorna un nuevo array con los resultados de la función
// Cuando me conviene utilizar un .map()? Cuando quiero modificar, es decir cuando quiera TRANSFORMAR los elementos de un array y obtener un nuevo array con los elementos modificados

// Como lo indica el nombre de la variable, solo voy a obtener los nombres de los estudiantes
const studentsNames = students.map((student) => {
    console.log("Dentro de la función map", student)

    return student.firstname

    // Transformo los undefined a "No tiene ciudad" o a Null
    // // Condicional ternario --> condicion ? true : false
    // return student.city ? student.city : "No tiene ciudad"
    // return student.city ? student.city : null
})

console.log("Veamos el resultado", studentsNames)



const studentsNamesAndLastnames = students.map((student) => {
    return {
...
console.log("Veamos el resultado", studentsNamesAndLastnames)


const basicInfoUsers = users.map((user) => {
    return {
        id: user.id,
        name: user.firstname,
        mail: user.email
...
console.log("A ver que paso con los users", basicInfoUsers)

// Filter function
// También devuelve un nuevo array

// REQUISITOS: Devolvemos solo los usuarios con id superior a 15
const usersWithIdGreaterThan15 = users.filter((user) => {
    return user.id > 15
})

console.log("Usuarios con id mayor a 15", usersWithIdGreaterThan15)


// Find function
// Busco al usuario con apellido "Brown"
...
const lastnameBrown = users.filter((user) => {
    return user.lastname === "Brown"
})

console.log("Usuarios con apellido Brown", lastnameBrown)
